...
for i in tqdm(range(len(periodoselecionado))):
    ...
    for a in range(len(myresult)):
        count += 1
        problema = myresult[count][0]
        uf = myresult[count][1]
        empresa = myresult[count][2]
        resolvida = myresult[count][3]
        naoresolvida = myresult[count][4]
        finalizadaAvaliada = myresult[count][5]
        finalizadaNaoAvaliada = myresult[count][6]
        periodo = periodoselecionado[i]
        sql1 = "INSERT INTO tblproblema(uf, problema, clienteId, resolvida, naoResolvida, finalizadaAvaliada, finalizadaNaoAvaliada, periodo) values(%s,%s,%s,%s,%s,%s,%s,%s)"
        val1 = (uf, problema, empresa, resolvida, naoresolvida, finalizadaAvaliada, finalizadaNaoAvaliada, periodo)
